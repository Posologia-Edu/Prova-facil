...
export default function MedicineEditor() {
  ...
  const mt = (moduleType || "anamnese_medica") as MedicineModuleType;
  const navigate = useNavigate();
  const queryClient = useQueryClient();

  const { data: room, isLoading: roomLoading } = useQuery({
    queryKey: ["medicine-room", roomId],
    queryFn: async () => {
      const { data, error } = await supabase.from("medicine_rooms").select("*").eq("id", roomId!).single();
  ...
  const { data: participants = [], refetch: refetchParticipants } = useQuery({
    queryKey: ["medicine-participants", roomId],
    queryFn: async () => {
      const { data, error } = await supabase.from("medicine_participants").select("*").eq("room_id", roomId!).order("pair_index", { ascending: true });
      if (error) throw error;
      return data;
    },
    enabled: !!roomId,
  ...
  const { data: forms = [], refetch: refetchForms } = useQuery({
    queryKey: ["medicine-forms", roomId],
    queryFn: async () => {
      const { data, error } = await supabase.from("medicine_forms").select("*").eq("room_id", roomId!).order("created_at", { ascending: true });
      if (error) throw error;
      return data as any[];
    },
    enabled: !!roomId,
  ...
  const { data: clinicalCases = [], refetch: refetchCases } = useQuery({
    queryKey: ["medicine-clinical-cases", roomId],
    queryFn: async () => {
      const { data, error } = await supabase.from("medicine_clinical_cases").select("*").eq("room_id", roomId!).order("position", { ascending: true });
      if (error) throw error;
      return data;
    },
    enabled: !!roomId,
  ...
  const addParticipant = async () => {
    ...
    const { error } = await supabase.from("medicine_participants").insert({
  ...
  const addCase = async () => {
    ...
    await supabase.from("medicine_clinical_cases").insert({ room_id: roomId!, title: newCaseTitle.trim(), content: newCaseContent.trim(), position: pos });
    setNewCaseTitle(""); setNewCaseContent(""); refetchCases();
    toast({ title: "Caso clínico adicionado" });
  };

  const deleteCase = async (id: string) => {
    await supabase.from("medicine_clinical_cases").delete().eq("id", id);
    refetchCases();
  };
  ...
  const saveForm = async (silent = false) => {
    ...
    if (editingFormId) {
      await supabase.from("medicine_forms").update(payload).eq("id", editingFormId);
    } else {
      await supabase.from("medicine_forms").insert({ ...payload, room_id: roomId! });
    }
  ...
  const deleteForm = async (id: string) => {
    await supabase.from("medicine_forms").delete().eq("id", id);
    refetchForms();
  };
  ...
  const activateRoom = async () => {
    ...
    await supabase.from("medicine_rooms").update({ status: "active" }).eq("id", roomId!);
    toast({ title: "Sala ativada!" });
    queryClient.invalidateQueries({ queryKey: ["medicine-room", roomId] });
  };
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <div className="flex items-center gap-2">
            <Badge variant="outline" className="text-xs bg-blue-600/10 text-blue-600 border-blue-600/30">{moduleLabel[mt]}</Badge>
        ...
        {room.status === "active" && <Button className="ml-auto" onClick={() => navigate(`/medicine/${mt}/control/${roomId}`)}><Play className="h-4 w-4 mr-1" />Painel de Controle</Button>}
      </div>
        ...
        <TabsContent value="participants" className="space-y-4">
          ...
          {(() => {
            ...
            const formPair = async () => {
              ...
              await supabase.from("medicine_participants").update({ pair_index: nextPairIdx, pair_position: "A" } as any).eq("id", a);
              await supabase.from("medicine_participants").update({ pair_index: nextPairIdx, pair_position: "B" } as any).eq("id", b);
            ...
            const markAsSolo = async () => {
              ...
              await supabase.from("medicine_participants").update({ pair_index: nextPairIdx, pair_position: "S" } as any).eq("id", selectedForPairing[0]);
            ...
            const undoPair = async (pairIdx: number) => {
              ...
              for (const m of members) await supabase.from("medicine_participants").update({ pair_index: -1, pair_position: "X" } as any).eq("id", m.id);
