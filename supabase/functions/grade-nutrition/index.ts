...
const moduleSystemPrompts: Record<string, string> = {
  anamnese_nutricional: `Você é um avaliador acadêmico de Nutrição especializado em Anamnese Nutricional.
Avalie as respostas considerando: histórico alimentar completo, hábitos alimentares, alergias e intolerâncias, preferências, uso de suplementos, consumo hídrico e padrão de refeições.`,
  avaliacao_antropometrica: `Você é um avaliador acadêmico de Nutrição especializado em Avaliação Antropométrica.
Avalie as respostas considerando: medidas corporais (peso, altura, circunferências), cálculo correto de IMC, interpretação de composição corporal, dobras cutâneas, indicadores nutricionais e classificação do estado nutricional.`,
  plano_alimentar: `Você é um avaliador acadêmico de Nutrição especializado em Plano Alimentar.
Avalie as respostas considerando: cálculo de necessidades energéticas (GEB/GET), distribuição adequada de macronutrientes, adequação de micronutrientes, fracionamento de refeições, lista de substituições e viabilidade do plano.`,
  orientacao_nutricional: `Você é um avaliador acadêmico de Nutrição especializado em Orientação Nutricional.
...
serve(async (req) => {
  ...
  try {
    ...
    if (response?.id) {
      ...
      await supabaseAdmin.from("nutrition_responses").update({ ai_score: totalScore, ai_feedback_json: { ...feedbackJson, feedback: grading.general_feedback } }).eq("id", response.id);
    }
    ...
    console.error("grade-nutrition error:", e);
