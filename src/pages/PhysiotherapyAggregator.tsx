...
export default function PhysiotherapyAggregator() {
  const navigate = useNavigate();

  const { data: rooms = [] } = useQuery({
    queryKey: ["physiotherapy-agg-rooms"],
    queryFn: async () => {
      const { data: { session } } = await supabase.auth.getSession();
      if (!session) return [];
      const { data } = await supabase.from("physiotherapy_rooms").select("id, title, status, module_type").eq("user_id", session.user.id).order("created_at", { ascending: false });
      return (data || []) as RoomInfo[];
    },
  });

  const { data: allResponses = [] } = useQuery({
    queryKey: ["physiotherapy-agg-responses"],
    queryFn: async () => {
      const { data } = await supabase.from("physiotherapy_responses").select("room_id, pair_index, admin_score, ai_score");
      return data || [];
    },
  });

  const { data: allParticipants = [] } = useQuery({
    queryKey: ["physiotherapy-agg-participants"],
    queryFn: async () => {
      const { data } = await supabase.from("physiotherapy_participants").select("room_id, pair_index, student_name, student_email, participant_role");
  ...
  const moduleIcons: Record<string, any> = { avaliacao_funcional: Heart, cinetico_funcional: ClipboardList, plano_fisioterapeutico: Activity, evolucao_fisio: ArrowRightLeft };

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" onClick={() => navigate("/physiotherapy")}><ArrowLeft className="h-4 w-4 mr-1" />Voltar</Button>
        <div>
          <h1 className="text-2xl font-bold">Agregador de Notas — Fisioterapia</h1>
