    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="participants">
        ...
        <TabsContent value="participants" className="space-y-4">
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">{Object.entries(pairs).map(([idx, pair]) => { const pairIdx = Number(idx); const hasResponse = responses.some(r => r.pair_index === pairIdx); return (<Card key={idx}><CardHeader className="pb-2"><div className="flex items-center justify-between"><CardTitle className="text-sm">Dupla {pairIdx + 1}</CardTitle><Badge variant={hasResponse ? "default" : "outline"}>{hasResponse ? "Enviou" : "Aguardando"}</Badge></div></CardHeader><CardContent>{pair.map((p: any) => <p key={p.id} className="text-sm">{p.student_name}</p>)}</CardContent></Card>); })}</div>
