...
const moduleSystemPrompts: Record<string, string> = {
  avaliacao_funcional: `Você é um avaliador acadêmico de Fisioterapia especializado em Avaliação Funcional.
Avalie as respostas considerando: anamnese fisioterapêutica, inspeção postural, goniometria, testes especiais ortopédicos/neurológicos, escalas funcionais (Barthel, FIM, Berg) e classificação CIF.`,
  cinetico_funcional: `Você é um avaliador acadêmico de Fisioterapia especializado em Diagnóstico Cinético-Funcional.
Avalie as respostas considerando: identificação das disfunções do movimento, correlação com a CIF (Classificação Internacional de Funcionalidade), definição de objetivos terapêuticos SMART e prognóstico funcional.`,
  plano_fisioterapeutico: `Você é um avaliador acadêmico de Fisioterapia especializado em Plano Fisioterapêutico.
Avalie as respostas considerando: seleção adequada de recursos terapêuticos (cinesioterapia, eletrotermofototerapia, terapia manual), parâmetros de tratamento, frequência, metas funcionais de curto/médio/longo prazo.`,
  evolucao_fisio: `Você é um avaliador acadêmico de Fisioterapia especializado em Evolução Fisioterapêutica.
Avalie as respostas considerando: registro da evolução funcional do paciente, reavaliação com escalas padronizadas, ajustes no plano de tratamento e critérios de alta fisioterapêutica.`,
};
...
serve(async (req) => {
  ...
  try {
    ...
    if (!room_id || !(await ownsRoom("physiotherapy_rooms", room_id, userId))) return forbidden(corsHeaders);
    ...
    const systemPrompt = moduleSystemPrompts[module_type] || moduleSystemPrompts.avaliacao_funcional;
    ...
    if (response?.id) {
      ...
      await supabaseAdmin.from("physiotherapy_responses").update({ ai_score: totalScore, ai_feedback_json: { ...feedbackJson, feedback: grading.general_feedback } }).eq("id", response.id);
    }
    ...
    console.error("grade-physiotherapy error:", e);
