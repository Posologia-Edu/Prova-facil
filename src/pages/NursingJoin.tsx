...
import { moduleLabel } from "@/lib/nursing-modules";

type Phase = "login" | "active" | "done";

export default function NursingJoin() {
  const [countdown, setCountdown] = useState(15);
  const [pin, setPin] = useState(() => sessionStorage.getItem("nursing_pin") || "");
  const [email, setEmail] = useState(() => sessionStorage.getItem("nursing_email") || "");
  ...
  const handleLogin = async () => {
    ...
    const { data: rooms } = await supabase.from("nursing_rooms").select("*").eq("access_code", pin.trim().toLowerCase()).eq("status", "active");
    ...
    const { data: parts } = await supabase.from("nursing_participants").select("*").eq("room_id", foundRoom.id).eq("student_email", email.trim().toLowerCase());
    if (!parts?.length) { toast({ title: "E-mail não encontrado", variant: "destructive" }); return; }
    const me = parts[0];
    setParticipant(me);
    await supabase.from("nursing_participants").update({ status: "ready" }).eq("id", me.id);

    if (me.pair_index >= 0) {
      const { data: partners } = await supabase.from("nursing_participants").select("*").eq("room_id", foundRoom.id).eq("pair_index", me.pair_index).neq("id", me.id);
      if (partners?.length) setPartner(partners[0]);
    }

    const { data: allForms } = await supabase.from("nursing_forms").select("*").eq("room_id", foundRoom.id);
    const sf = allForms?.find((f: any) => f.form_type === "standard");
    if (sf) setStandardForm(sf);

    const { data: cases } = await supabase.from("nursing_clinical_cases").select("*").eq("room_id", foundRoom.id).order("position", { ascending: true });
    if (cases?.length && me.pair_index >= 0) setClinicalCase(cases[me.pair_index % cases.length]);

    const { data: existingResp } = await supabase.from("nursing_responses").select("*").eq("room_id", foundRoom.id).eq("pair_index", me.pair_index);
    if (existingResp?.some((r: any) => r.submitted_at)) { setSubmitted(true); setPhase("done"); } else { setPhase("active"); }

    sessionStorage.setItem("nursing_pin", pin.trim().toLowerCase());
    sessionStorage.setItem("nursing_email", email.trim().toLowerCase());
  };

  const handleSubmit = async () => {
    if (!room || !participant || !standardForm) return;
    await supabase.from("nursing_responses").insert({
    ...
    await supabase.from("nursing_participants").update({ status: "done" }).eq("room_id", room.id).eq("pair_index", participant.pair_index);
      ...
      <div className="min-h-screen flex items-center justify-center bg-background p-4">
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            <CardTitle className="text-xl">Enfermagem Clínica</CardTitle>
