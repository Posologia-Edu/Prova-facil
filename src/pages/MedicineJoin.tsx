...
import { moduleLabel } from "@/lib/medicine-modules";

type Phase = "login" | "active" | "done";

export default function MedicineJoin() {
  const [countdown, setCountdown] = useState(15);
  const [pin, setPin] = useState(() => sessionStorage.getItem("medicine_pin") || "");
  const [email, setEmail] = useState(() => sessionStorage.getItem("medicine_email") || "");
  ...
  const handleLogin = async () => {
    ...
    const { data: rooms } = await supabase.from("medicine_rooms").select("*").eq("access_code", pin.trim().toLowerCase()).eq("status", "active");
    ...
    const { data: parts } = await supabase.from("medicine_participants").select("*").eq("room_id", foundRoom.id).eq("student_email", email.trim().toLowerCase());
    if (!parts?.length) { toast({ title: "E-mail não encontrado", variant: "destructive" }); return; }
    const me = parts[0];
    setParticipant(me);
    await supabase.from("medicine_participants").update({ status: "ready" }).eq("id", me.id);

    if (me.pair_index >= 0 && me.pair_position !== "S") {
      const { data: partners } = await supabase.from("medicine_participants").select("*").eq("room_id", foundRoom.id).eq("pair_index", me.pair_index).neq("id", me.id);
      if (partners?.length) setPartner(partners[0]);
    }

    const { data: allForms } = await supabase.from("medicine_forms").select("*").eq("room_id", foundRoom.id);
    const sf = allForms?.find((f: any) => f.form_type === "standard");
    if (sf) setStandardForm(sf);

    const { data: cases } = await supabase.from("medicine_clinical_cases").select("*").eq("room_id", foundRoom.id).order("position", { ascending: true });
    if (cases?.length && me.pair_index >= 0) setClinicalCase(cases[me.pair_index % cases.length]);

    const { data: existingResp } = await supabase.from("medicine_responses").select("*").eq("room_id", foundRoom.id).eq("pair_index", me.pair_index);
    if (existingResp?.some((r: any) => r.submitted_at)) { setSubmitted(true); setPhase("done"); } else { setPhase("active"); }

    sessionStorage.setItem("medicine_pin", pin.trim().toLowerCase());
    sessionStorage.setItem("medicine_email", email.trim().toLowerCase());
  };
  ...
  const handleSubmit = async () => {
    ...
    await supabase.from("medicine_responses").insert({
    ...
    await supabase.from("medicine_participants").update({ status: "done" }).eq("room_id", room.id).eq("pair_index", participant.pair_index);
      ...
      <div className="min-h-screen flex items-center justify-center bg-background p-4">
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            <CardTitle className="text-xl">Medicina</CardTitle>
  ...
  return (
    <div className="min-h-screen bg-background p-4">
      <div className="max-w-3xl mx-auto space-y-6">
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">{moduleLabel[room?.module_type] || "Medicina"} — {room?.title}</CardTitle>
