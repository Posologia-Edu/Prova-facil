                    ...
                    <CardContent className="p-4 pt-0">
                      ...
                      <div className="flex gap-2 mt-3">
                        ...
                        {selectedForPairing.length === 1 && (
                          <Button onClick={() => { const markAsSolo = async () => { const nextIdx = paired.length > 0 ? Math.max(0, ...paired.map(p => p.pair_index)) + 1 : 0; await supabase.from("documentation_participants").update({ pair_index: nextIdx, pair_position: "S" } as any).eq("id", selectedForPairing[0]); setSelectedForPairing([]); refetchParticipants(); toast({ title: "Marcado como individual!" }); }; markAsSolo(); }} variant="outline" className="flex-1" size="sm">
