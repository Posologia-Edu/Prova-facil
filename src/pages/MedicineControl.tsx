...
export default function MedicineControl() {
  ...
  const { data: room } = useQuery({
    ...
    queryFn: async () => { const { data, error } = await supabase.from("medicine_rooms").select("*").eq("id", roomId!).single(); if (error) throw error; return data; },
    enabled: !!roomId,
  });

  const { data: participants = [] } = useQuery({
    queryKey: ["medicine-participants", roomId],
    queryFn: async () => { const { data, error } = await supabase.from("medicine_participants").select("*").eq("room_id", roomId!).order("pair_index", { ascending: true }); if (error) throw error; return data; },
    enabled: !!roomId, refetchInterval: 5000,
  });

  const { data: responses = [] } = useQuery({
    queryKey: ["medicine-responses", roomId],
    queryFn: async () => { const { data, error } = await supabase.from("medicine_responses").select("*").eq("room_id", roomId!); if (error) throw error; return data as any[]; },
    enabled: !!roomId, refetchInterval: 5000,
  });

  const { data: forms = [] } = useQuery({
    queryKey: ["medicine-forms", roomId],
    queryFn: async () => { const { data, error } = await supabase.from("medicine_forms").select("*").eq("room_id", roomId!); if (error) throw error; return data as any[]; },
    enabled: !!roomId,
  });

  const { data: clinicalCases = [] } = useQuery({
    queryKey: ["medicine-clinical-cases", roomId],
    queryFn: async () => { const { data, error } = await supabase.from("medicine_clinical_cases").select("*").eq("room_id", roomId!).order("position", { ascending: true }); if (error) throw error; return data; },
    enabled: !!roomId,
  });
  ...
  const saveAdminEvaluation = async () => {
    if (selectedResp) {
      await supabase.from("medicine_responses").update({ admin_score: adminScore ? Number(adminScore) : null, admin_feedback: adminFeedback || null }).eq("id", selectedResp.id);
    }
    toast({ title: "Avaliação salva!" });
    queryClient.invalidateQueries({ queryKey: ["medicine-responses", roomId] });
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" onClick={() => navigate(`/medicine/${mt}/editor/${roomId}`)}><ArrowLeft className="h-4 w-4 mr-1" />Voltar</Button>
        <div>
          ...
          {room?.status === "active" && responses.length > 0 && (
            <div className="flex items-center justify-between p-4 border rounded-lg bg-muted/50">
              ...
              <Button variant="default" onClick={async () => {
                await supabase.from("medicine_rooms").update({ status: "completed" }).eq("id", roomId!);
