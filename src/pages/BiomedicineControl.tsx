...
function CollapsibleAnswerField({ field }: { field: any }) {
  ...
  return (
    <Collapsible open={isOpen} onOpenChange={setIsOpen} className="mb-2">
      ...
      <CollapsibleContent>
        <div className="text-sm bg-green-50 dark:bg-green-950 p-3 rounded-b border border-t-0 border-green-200 dark:border-green-800">{answerText}</div>
      </CollapsibleContent>
    </Collapsible>
  );
}

export default function BiomedicineControl() {
  ...
  const { data: room } = useQuery({
    ...
    queryFn: async () => { const { data, error } = await supabase.from("biomedicine_rooms").select("*").eq("id", roomId!).single(); if (error) throw error; return data; },
    enabled: !!roomId,
  });

  const { data: participants = [] } = useQuery({
    queryKey: ["biomedicine-participants", roomId],
    queryFn: async () => { const { data, error } = await supabase.from("biomedicine_participants").select("*").eq("room_id", roomId!).order("pair_index", { ascending: true }); if (error) throw error; return data; },
    enabled: !!roomId, refetchInterval: 5000,
  });

  const { data: responses = [] } = useQuery({
    queryKey: ["biomedicine-responses", roomId],
    queryFn: async () => { const { data, error } = await supabase.from("biomedicine_responses").select("*").eq("room_id", roomId!); if (error) throw error; return data as any[]; },
    enabled: !!roomId, refetchInterval: 5000,
  });

  const { data: forms = [] } = useQuery({
    queryKey: ["biomedicine-forms", roomId],
    queryFn: async () => { const { data, error } = await supabase.from("biomedicine_forms").select("*").eq("room_id", roomId!); if (error) throw error; return data as any[]; },
    enabled: !!roomId,
  });

  const { data: clinicalCases = [] } = useQuery({
    queryKey: ["biomedicine-clinical-cases", roomId],
    queryFn: async () => { const { data, error } = await supabase.from("biomedicine_clinical_cases").select("*").eq("room_id", roomId!).order("position", { ascending: true }); if (error) throw error; return data; },
    enabled: !!roomId,
  });
  ...
  const saveAdminEvaluation = async () => {
    if (selectedResp) {
      await supabase.from("biomedicine_responses").update({ admin_score: adminScore ? Number(adminScore) : null, admin_feedback: adminFeedback || null }).eq("id", selectedResp.id);
    }
    toast({ title: "Avaliação salva!" });
    queryClient.invalidateQueries({ queryKey: ["biomedicine-responses", roomId] });
  };
  ...
  const gradeWithAI = async () => {
    ...
    setGradingAI(true);
    try {
      const { data, error } = await supabase.functions.invoke("grade-biomedicine", {
        body: {
          ...
          answer_key: answerKeyForm?.content_json || null,
          form_fields: standardForm?.content_json || [],
        },
      });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" onClick={() => navigate(`/biomedicine/${mt}/editor/${roomId}`)}><ArrowLeft className="h-4 w-4 mr-1" />Voltar</Button>
        <div>
            ...
            {Object.entries(pairs).map(([idx, pair]) => {
              ...
              return (
                <Card key={idx}>
                  ...
                  <CardContent>{pair.map((p: any) => <p key={p.id} className="text-sm">{p.student_name}</p>)}</CardContent>
                </Card>
              );
            })}
          </div>
          {room?.status === "active" && responses.length > 0 && (
            <div className="flex items-center justify-between p-4 border rounded-lg bg-muted/50">
              ...
              <Button variant="default" onClick={async () => {
                await supabase.from("biomedicine_rooms").update({ status: "completed" }).eq("id", roomId!);
                queryClient.invalidateQueries({ queryKey: ["biomedicine-room", roomId] });
                toast({ title: "Sala concluída!" });
              }}><Lock className="h-4 w-4 mr-1" />Concluir Sala</Button>
            </div>
          )}
                ...
                <CardContent className="space-y-4">
                      ...
                      <div>
                        ...
                        {standardFields.map(field => (
                          <div key={field.id} className="mb-2">
                            ...
                            <p className="text-sm bg-muted p-2 rounded">{renderAnswerValue((resp.answers_json as any)?.[field.id])}</p>
                          </div>
                        ))}
                      </div>
                      <div>
                        <p className="text-xs font-semibold text-muted-foreground mb-2">Espelho</p>
                        {caseKeyFields.length ? caseKeyFields.map((f: any) => <CollapsibleAnswerField key={f.id} field={f} />) : <p className="text-sm text-muted-foreground">Sem espelho cadastrado.</p>}
                      </div>
                    </div>
                  )}
                  {resp?.ai_feedback_json && (
                    <div className="p-3 bg-blue-50 dark:bg-blue-950 rounded border border-blue-200 dark:border-blue-800">
                      ...
                      <p className="text-sm">{typeof resp.ai_feedback_json === "string" ? resp.ai_feedback_json : (resp.ai_feedback_json as any)?.feedback || JSON.stringify(resp.ai_feedback_json)}</p>
                      {resp.ai_score != null && <p className="font-medium text-sm mt-2">Nota IA: {resp.ai_score}/10,0</p>}
                    </div>
                  )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="space-y-2">
              ...
              {pairIndicesWithResponses.map(pairIdx => {
                const resp = responses.find(r => r.pair_index === pairIdx && r.form_id === standardForm?.id);
                return (
                  <Button key={pairIdx} variant={selectedPairIndex === pairIdx ? "default" : "outline"} className="w-full justify-start" size="sm" onClick={() => handleSelectPair(pairIdx)}>
                    {pairNames(pairIdx)}
                    {resp?.admin_score != null && <Badge className="ml-auto" variant="secondary">{resp.admin_score}</Badge>}
                  </Button>
                );
              })}
            </div>
