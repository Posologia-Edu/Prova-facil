...
interface SplitSoapRoomDialogProps {
  roomId: string;
  open: boolean;
...
export default function SplitSoapRoomDialog({ roomId, open, onOpenChange, onComplete }: SplitSoapRoomDialogProps) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const [{ data: roomData }, { data: parts }, { data: formsData }] = await Promise.all([
        supabase.from("soap_rooms").select("*").eq("id", roomId).single(),
        supabase.from("soap_participants").select("*").eq("room_id", roomId),
        supabase.from("soap_forms").select("*").eq("room_id", roomId),
      ]);
      setRoom(roomData);
  ...
  const remainingStudents = participants.filter((s) => !assignedStudentIds.includes(s.id));
  ...
  const handleSplit = async () => {
    ...
    try {
      ...
      for (const sr of subRooms) {
        const { data: newRoom, error: roomErr } = await supabase
          ...
          .insert({
            ...
            anamnesis_room_id: room.anamnesis_room_id,
            status: "draft",
          })
          .select()
          .single();
      ...
      await supabase.from("soap_responses").delete().eq("room_id", roomId);
      await supabase.from("soap_forms").delete().eq("room_id", roomId);
      await supabase.from("soap_participants").delete().eq("room_id", roomId);
      await supabase.from("soap_rooms").delete().eq("id", roomId);
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Dividir Sala SOAP — {room?.title}</DialogTitle>
        </DialogHeader>
            ...
            <div key={idx} className="border rounded-lg p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Input value={sr.professorName} onChange={(e) => updateSubRoom(idx, "professorName", e.target.value)} placeholder="Prof. Silva" />
                </div>
                <div>
                  <Label className="text-xs">Email do Professor</Label>
                  <Input value={sr.professorEmail} onChange={(e) => updateSubRoom(idx, "professorEmail", e.target.value)} placeholder="professor@email.com" />
                </div>
              </div>
              <div>
                ...
                <div className="grid grid-cols-2 gap-1 mt-1 max-h-40 overflow-y-auto">
                  {participants.map((s) => {
                    ...
                    return (
                      <label key={s.id} className={`flex items-center gap-2 p-1.5 rounded text-sm cursor-pointer hover:bg-muted ${assignedElsewhere ? "opacity-40 pointer-events-none" : ""}`}>
                        <Checkbox checked={sr.studentIds.includes(s.id)} onCheckedChange={() => toggleStudent(idx, s.id)} disabled={assignedElsewhere} />
                        {s.student_name}
                      </label>
                    );
                  })}
