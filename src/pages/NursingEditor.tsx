...
import { moduleLabel, type NursingModuleType } from "@/lib/nursing-modules";
...
export default function NursingEditor() {
  const { roomId, moduleType } = useParams<{ roomId: string; moduleType: string }>();
  const mt = (moduleType || "acolhimento") as NursingModuleType;
  const navigate = useNavigate();
  const queryClient = useQueryClient();

  const { data: room, isLoading: roomLoading } = useQuery({
    queryKey: ["nursing-room", roomId],
    queryFn: async () => {
      const { data, error } = await supabase.from("nursing_rooms").select("*").eq("id", roomId!).single();
  ...
  const { data: participants = [], refetch: refetchParticipants } = useQuery({
    queryKey: ["nursing-participants", roomId],
    queryFn: async () => {
      const { data, error } = await supabase.from("nursing_participants").select("*").eq("room_id", roomId!).order("pair_index", { ascending: true });
      if (error) throw error;
      return data;
    },
    enabled: !!roomId,
  ...
  const { data: forms = [], refetch: refetchForms } = useQuery({
    queryKey: ["nursing-forms", roomId],
    queryFn: async () => {
      const { data, error } = await supabase.from("nursing_forms").select("*").eq("room_id", roomId!).order("created_at", { ascending: true });
      if (error) throw error;
      return data as any[];
    },
    enabled: !!roomId,
  ...
  const { data: clinicalCases = [], refetch: refetchCases } = useQuery({
    queryKey: ["nursing-clinical-cases", roomId],
    queryFn: async () => {
      const { data, error } = await supabase.from("nursing_clinical_cases").select("*").eq("room_id", roomId!).order("position", { ascending: true });
      if (error) throw error;
      return data;
    },
    enabled: !!roomId,
  ...
  const addParticipant = async () => {
    ...
    const { error } = await supabase.from("nursing_participants").insert({
  ...
  const addCase = async () => {
    ...
    await supabase.from("nursing_clinical_cases").insert({ room_id: roomId!, title: newCaseTitle.trim(), content: newCaseContent.trim(), position: pos });
    setNewCaseTitle(""); setNewCaseContent(""); refetchCases();
    toast({ title: "Caso clínico adicionado" });
  };

  const deleteCase = async (id: string) => {
    await supabase.from("nursing_clinical_cases").delete().eq("id", id);
    refetchCases();
  };
  ...
  const saveForm = async (silent = false) => {
    ...
    if (editingFormId) {
      await supabase.from("nursing_forms").update(payload).eq("id", editingFormId);
    } else {
      await supabase.from("nursing_forms").insert({ ...payload, room_id: roomId! });
    }
  ...
  const deleteForm = async (id: string) => {
    await supabase.from("nursing_forms").delete().eq("id", id);
    refetchForms();
  };
  ...
  const activateRoom = async () => {
    ...
    await supabase.from("nursing_rooms").update({ status: "active" }).eq("id", roomId!);
    toast({ title: "Sala ativada!" });
    queryClient.invalidateQueries({ queryKey: ["nursing-room", roomId] });
  };
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <div className="flex items-center gap-2">
            <Badge variant="outline" className="text-xs bg-rose-500/10 text-rose-500 border-rose-500/30">{moduleLabel[mt]}</Badge>
        ...
        {room.status === "active" && <Button className="ml-auto" onClick={() => navigate(`/nursing/${mt}/control/${roomId}`)}><Play className="h-4 w-4 mr-1" />Painel de Controle</Button>}
      </div>
      ...
      <Tabs defaultValue="participants">
        ...
        <TabsContent value="participants" className="space-y-4">
          ...
          {(() => {
            ...
            const formPair = async () => {
              ...
              await supabase.from("nursing_participants").update({ pair_index: nextPairIdx, pair_position: "A" } as any).eq("id", a);
              await supabase.from("nursing_participants").update({ pair_index: nextPairIdx, pair_position: "B" } as any).eq("id", b);
        ...
        <TabsContent value="forms" className="space-y-4">
          <Button variant="outline" onClick={() => setTemplateDialogOpen(true)}>
            <Star className="h-4 w-4 mr-2" />Usar Template
          </Button>
              ...
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-1">
                    <Button variant="ghost" size="sm" onClick={() => { setSaveTemplateForm(form); setSaveTemplateDialogOpen(true); }} title="Salvar como Template">
