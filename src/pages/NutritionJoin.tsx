...
import { nutritionModuleLabel } from "@/lib/nutrition-modules";

type Phase = "login" | "active" | "done";

export default function NutritionJoin() {
  const [countdown, setCountdown] = useState(15);
  const [pin, setPin] = useState(() => sessionStorage.getItem("nutrition_pin") || "");
  const [email, setEmail] = useState(() => sessionStorage.getItem("nutrition_email") || "");
  ...
  const handleLogin = async () => {
    ...
    const { data: rooms } = await supabase.from("nutrition_rooms").select("*").eq("access_code", pin.trim().toLowerCase()).eq("status", "active");
    if (!rooms?.length) { toast({ title: "Sala não encontrada", variant: "destructive" }); return; }
    const foundRoom = rooms[0]; setRoom(foundRoom);
    const { data: parts } = await supabase.from("nutrition_participants").select("*").eq("room_id", foundRoom.id).eq("student_email", email.trim().toLowerCase());
    ...
    const { data: existingResp } = await supabase.from("nutrition_responses").select("*").eq("room_id", foundRoom.id).eq("pair_index", me.pair_index);
    if (existingResp?.some((r: any) => r.submitted_at)) { setSubmitted(true); setPhase("done"); } else { setPhase("active"); }
    sessionStorage.setItem("nutrition_pin", pin.trim().toLowerCase());
    sessionStorage.setItem("nutrition_email", email.trim().toLowerCase());
  };
  ...
  if (phase === "login") return (
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <Card className="w-full max-w-md"><CardHeader className="text-center"><CardTitle className="text-xl">Nutrição Clínica</CardTitle><p className="text-sm text-muted-foreground">Insira o PIN e seu e-mail para entrar</p></CardHeader>
