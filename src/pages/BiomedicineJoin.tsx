...
import { moduleLabel } from "@/lib/biomedicine-modules";

type Phase = "login" | "active" | "done";

export default function BiomedicineJoin() {
  const [countdown, setCountdown] = useState(15);
  const [pin, setPin] = useState(() => sessionStorage.getItem("biomedicine_pin") || "");
  const [email, setEmail] = useState(() => sessionStorage.getItem("biomedicine_email") || "");
  ...
  const handleLogin = async () => {
    ...
    const { data: rooms } = await supabase.from("biomedicine_rooms").select("*").eq("access_code", pin.trim().toLowerCase()).eq("status", "active");
    ...
    const { data: parts } = await supabase.from("biomedicine_participants").select("*").eq("room_id", foundRoom.id).eq("student_email", email.trim().toLowerCase());
    ...
    const { data: allForms } = await supabase.from("biomedicine_forms").select("*").eq("room_id", foundRoom.id);
    const sf = allForms?.find((f: any) => f.form_type === "standard");
    if (sf) setStandardForm(sf);

    const { data: cases } = await supabase.from("biomedicine_clinical_cases").select("*").eq("room_id", foundRoom.id).order("position", { ascending: true });
    if (cases?.length && me.pair_index >= 0) setClinicalCase(cases[me.pair_index % cases.length]);

    const { data: existingResp } = await supabase.from("biomedicine_responses").select("*").eq("room_id", foundRoom.id).eq("pair_index", me.pair_index);
    if (existingResp?.some((r: any) => r.submitted_at)) { setSubmitted(true); setPhase("done"); } else { setPhase("active"); }

    sessionStorage.setItem("biomedicine_pin", pin.trim().toLowerCase());
    sessionStorage.setItem("biomedicine_email", email.trim().toLowerCase());
  };
  ...
  const handleSubmit = async () => {
    ...
    await supabase.from("biomedicine_participants").update({ status: "done" }).eq("room_id", room.id).eq("pair_index", participant.pair_index);
      ...
      <div className="min-h-screen flex items-center justify-center bg-background p-4">
        <Card className="w-full max-w-md">
          <CardHeader className="text-center">
            <CardTitle className="text-xl">Biomedicina</CardTitle>
