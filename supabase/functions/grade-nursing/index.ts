...
const moduleSystemPrompts: Record<string, string> = {
  acolhimento: `Você é um avaliador acadêmico de Enfermagem especializado em Acolhimento com Classificação de Risco.
Avalie as respostas considerando: coleta de dados do paciente, identificação da queixa principal, aferição de sinais vitais, classificação de risco (Protocolo de Manchester ou similar) e encaminhamento adequado.`,
  sae: `Você é um avaliador acadêmico de Enfermagem especializado em SAE (Sistematização da Assistência de Enfermagem).
Avalie as respostas considerando as 5 etapas do Processo de Enfermagem: Histórico, Diagnóstico de Enfermagem (NANDA), Planejamento (NIC), Implementação e Avaliação (NOC).`,
  evolucao: `Você é um avaliador acadêmico de Enfermagem especializado em Evolução de Enfermagem.
Avalie as respostas considerando o registro cronológico da evolução do paciente, uso adequado do formato SOAP adaptado para enfermagem, objetividade, completude e pertinência clínica.`,
  passagem_plantao: `Você é um avaliador acadêmico de Enfermagem especializado em Passagem de Plantão.
...
serve(async (req) => {
  ...
  try {
    ...
    if (response && answer_key) {
      let keyFields: any[] = [];
      if (answer_key.case_answers) {
        const caseId = response.clinical_case_id;
        if (caseId && answer_key.case_answers[caseId]) keyFields = answer_key.case_answers[caseId];
        else { const firstKey = Object.keys(answer_key.case_answers)[0]; if (firstKey) keyFields = answer_key.case_answers[firstKey]; }
    ...
    if (response?.id) {
      ...
      await supabaseAdmin.from("nursing_responses").update({
        ai_score: totalScore,
        ai_feedback_json: { ...feedbackJson, feedback: grading.general_feedback },
      }).eq("id", response.id);
    }
