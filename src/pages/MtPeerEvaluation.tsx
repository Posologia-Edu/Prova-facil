...
import { Loader2, Gavel, CheckCircle2, Clock, ShieldAlert } from "lucide-react";
import { supabase } from "@/integrations/supabase/client";
import { toast } from "sonner";
import { getMtPeerEvalCriteria, MtPeerEvaluationScores } from "@/lib/mt-peer-eval";
...
type FormState = Record<string, Partial<MtPeerEvaluationScores> & { comentario?: string }>;

export default function MtPeerEvaluation() {
  ...
  const load = async () => {
    ...
    setStatus("loading");
    try {
      const { data, error } = await supabase.functions.invoke("mt-peer-eval-context", { body: { token } });
      if (error) throw error;
      setEvaluatorName(data?.evaluator_name || "");
      if (data?.status === "pending") {
        setCaseLabel(data.case_label || "");
        setTrialLabel(data.trial_label || "");
  ...
  const criteria = getMtPeerEvalCriteria(role);

  const updateScore = (email: string, key: keyof MtPeerEvaluationScores, value: number) => {
  ...
  const isComplete = teammates.every((t) => {
    ...
    return f && criteria.every((c) => typeof f[c.key] === "number");
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      const ratings = teammates.map((t) => ({
        ...
        preparacao_score: form[t.email]?.preparacao_score,
        atuacao_score: form[t.email]?.atuacao_score,
        colaboracao_score: form[t.email]?.colaboracao_score,
        comentario: form[t.email]?.comentario || "",
      }));
      const { data, error } = await supabase.functions.invoke("submit-mt-peer-evaluation", {
        body: { token, ratings },
      });
  ...
  if (status === "used" || status === "submitted") {
    return (
      <StatusCard
        ...
        description="Obrigado! Sua avaliação entre pares para este processo já foi registrada."
      />
    );
  }
  ...
  if (status === "expired") {
    return (
      <StatusCard
        ...
        title="Link expirado"
        description="O prazo para avaliar seus colegas neste processo encerrou. Fale com seu professor se precisar de mais tempo."
      />
    );
  }
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        <div>
          <h1 className="text-xl font-semibold flex items-center gap-2">
            <Gavel className="h-5 w-5 text-primary" /> Avaliação entre pares
          </h1>
          <div className="flex items-center gap-2 mt-1.5 flex-wrap">
            <Badge variant="outline">{trialLabel}</Badge>
            <Badge variant="outline">{caseLabel}</Badge>
            <Badge>{roleLabel}</Badge>
          </div>
          <p className="text-sm text-muted-foreground mt-2">
            Olá, {evaluatorName || "colega"}! Avalie a participação de cada colega do seu grupo neste processo
            específico ({caseLabel}, atuando como {roleLabel}). Suas respostas não são mostradas aos colegas avaliados.
          </p>
        </div>
