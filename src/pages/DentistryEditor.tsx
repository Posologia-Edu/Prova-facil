...
export default function DentistryEditor() {
  ...
  const { data: room, isLoading: roomLoading } = useQuery({ queryKey: ["dentistry-room", roomId], queryFn: async () => { const { data, error } = await supabase.from("dentistry_rooms").select("*").eq("id", roomId!).single(); if (error) throw error; return data; }, enabled: !!roomId });
  const { data: participants = [], refetch: refetchParticipants } = useQuery({ queryKey: ["dentistry-participants", roomId], queryFn: async () => { const { data, error } = await supabase.from("dentistry_participants").select("*").eq("room_id", roomId!).order("pair_index", { ascending: true }); if (error) throw error; return data; }, enabled: !!roomId });
  const { data: forms = [], refetch: refetchForms } = useQuery({ queryKey: ["dentistry-forms", roomId], queryFn: async () => { const { data, error } = await supabase.from("dentistry_forms").select("*").eq("room_id", roomId!).order("created_at", { ascending: true }); if (error) throw error; return data as any[]; }, enabled: !!roomId });
  const { data: clinicalCases = [], refetch: refetchCases } = useQuery({ queryKey: ["dentistry-clinical-cases", roomId], queryFn: async () => { const { data, error } = await supabase.from("dentistry_clinical_cases").select("*").eq("room_id", roomId!).order("position", { ascending: true }); if (error) throw error; return data; }, enabled: !!roomId });
  ...
  const addParticipant = async () => {
    ...
    await supabase.from("dentistry_participants").insert({ room_id: roomId!, student_name: newName.trim(), student_email: newEmail.trim(), pair_index: -1, pair_position: "X", participant_role: "student" });
    setNewName(""); setNewEmail(""); refetchParticipants();
  };

  const addCase = async () => {
    if (!newCaseTitle.trim()) return;
    await supabase.from("dentistry_clinical_cases").insert({ room_id: roomId!, title: newCaseTitle.trim(), content: newCaseContent.trim(), position: clinicalCases.length });
    setNewCaseTitle(""); setNewCaseContent(""); refetchCases(); toast({ title: "Caso clínico adicionado" });
  };

  const deleteCase = async (id: string) => { await supabase.from("dentistry_clinical_cases").delete().eq("id", id); refetchCases(); };

  const saveForm = async (silent = false) => {
    const normalizedTitle = formTitle.trim(); if (!normalizedTitle) return;
    ...
    if (editingFormId) { await supabase.from("dentistry_forms").update(payload).eq("id", editingFormId); }
    else { await supabase.from("dentistry_forms").insert({ ...payload, room_id: roomId! }); }
  ...
  const editForm = (form: any) => {
    skipNextAutoSaveRef.current = true; setEditingFormId(form.id); setFormTitle(form.title); setFormType(form.form_type);
  ...
  const deleteForm = async (id: string) => { await supabase.from("dentistry_forms").delete().eq("id", id); refetchForms(); };

  const activateRoom = async () => {
    const standardForm = forms.find((f: any) => f.form_type === "standard");
    if (!standardForm) { toast({ title: "Formulário necessário", variant: "destructive" }); return; }
    if (!students.length) { toast({ title: "Sem alunos", variant: "destructive" }); return; }
    await supabase.from("dentistry_rooms").update({ status: "active" }).eq("id", roomId!);
    toast({ title: "Sala ativada!" }); queryClient.invalidateQueries({ queryKey: ["dentistry-room", roomId] });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <div className="flex items-center gap-2"><Badge variant="outline" className="text-xs bg-cyan-500/10 text-cyan-500 border-cyan-500/30">{dentistryModuleLabel[mt]}</Badge><h1 className="text-xl font-bold">{room.title}</h1></div>
        ...
        {room.status === "active" && <Button className="ml-auto" onClick={() => navigate(`/dentistry/${mt}/control/${roomId}`)}><Play className="h-4 w-4 mr-1" />Painel de Controle</Button>}
      </div>

      <Tabs defaultValue="participants">
        <TabsList><TabsTrigger value="participants"><Users className="h-4 w-4 mr-1" />Participantes</TabsTrigger><TabsTrigger value="forms"><FileText className="h-4 w-4 mr-1" />Formulários</TabsTrigger><TabsTrigger value="cases"><BookOpen className="h-4 w-4 mr-1" />Casos Clínicos</TabsTrigger></TabsList>

        <TabsContent value="participants" className="space-y-4">
          <Card><CardHeader><CardTitle className="text-base">Adicionar Aluno</CardTitle></CardHeader><CardContent><div className="flex gap-2"><Input placeholder="Nome" value={newName} onChange={e => setNewName(e.target.value)} /><Input placeholder="E-mail" value={newEmail} onChange={e => setNewEmail(e.target.value)} /><Button onClick={addParticipant} disabled={!newName.trim()}><Plus className="h-4 w-4" /></Button></div></CardContent></Card>
          {(() => {
            ...
            const formPair = async () => {
              ...
              await supabase.from("dentistry_participants").update({ pair_index: nextPairIdx, pair_position: "A" } as any).eq("id", a);
              await supabase.from("dentistry_participants").update({ pair_index: nextPairIdx, pair_position: "B" } as any).eq("id", b);
            ...
            const markAsSolo = async () => {
              ...
              await supabase.from("dentistry_participants").update({ pair_index: nextPairIdx, pair_position: "S" } as any).eq("id", selectedForPairing[0]);
              setSelectedForPairing([]); refetchParticipants(); toast({ title: "Marcado como individual!" });
            };
            const undoPair = async (pairIdx: number) => { const members = pairGroups[pairIdx] || []; for (const m of members) await supabase.from("dentistry_participants").update({ pair_index: -1, pair_position: "X" } as any).eq("id", m.id); refetchParticipants(); };
            return (
              <>
                {Object.keys(pairGroups).length > 0 && (<Card><CardHeader className="pb-2"><CardTitle className="text-base flex items-center gap-2">Duplas Formadas <Badge variant="secondary">{Object.keys(pairGroups).length}</Badge></CardTitle></CardHeader><CardContent className="p-4 pt-0"><div className="space-y-2">{Object.entries(pairGroups).map(([idx, members]) => { const isSolo = members.length === 1 && members[0].pair_position === "S"; return (<div key={idx} className="flex items-center justify-between py-2 px-3 rounded-lg bg-primary/5 border border-primary/10"><div className="flex items-center gap-3"><Badge variant="outline" className={isSolo ? "bg-amber-100 text-amber-800 border-amber-300 dark:bg-amber-900 dark:text-amber-200" : ""}>{isSolo ? "Individual" : `Dupla ${Number(idx) + 1}`}</Badge>{members.map(m => <span key={m.id} className="text-sm"><span className="font-medium">{m.student_name}</span>{!isSolo && <span className="text-muted-foreground ml-1">({m.pair_position})</span>}</span>)}</div><Button variant="ghost" size="icon" onClick={() => undoPair(Number(idx))}><RotateCcw className="h-4 w-4" /></Button></div>); })}</div></CardContent></Card>)}
                {unpaired.length > 0 && (<Card><CardHeader className="pb-2"><CardTitle className="text-base flex items-center gap-2">Alunos sem dupla <Badge variant="secondary">{unpaired.length}</Badge></CardTitle><p className="text-sm text-muted-foreground">Selecione 2 alunos para formar uma dupla ou 1 para marcar como individual</p></CardHeader><CardContent className="p-4 pt-0"><div className="grid grid-cols-2 gap-2">{unpaired.map(p => { const isSelected = selectedForPairing.includes(p.id); return (<button key={p.id} onClick={() => toggleSelect(p.id)} className={`p-3 rounded-lg border text-left text-sm transition-colors ${isSelected ? "border-primary bg-primary/10 ring-2 ring-primary" : "border-border hover:border-primary/50"}`}><span className="font-medium">{p.student_name}</span>{p.student_email && <p className="text-xs text-muted-foreground">{p.student_email}</p>}</button>); })}</div><div className="flex gap-2 mt-3">{selectedForPairing.length === 2 && <Button onClick={formPair} className="flex-1" size="sm"><Users className="h-4 w-4 mr-1" />Formar Dupla</Button>}{selectedForPairing.length === 1 && <Button onClick={markAsSolo} variant="outline" className="flex-1" size="sm">Marcar como Individual</Button>}</div></CardContent></Card>)}
        ...
        <TabsContent value="forms" className="space-y-4">
          ...
          <Card><CardHeader><CardTitle className="text-base">{editingFormId ? "Editando Formulário" : "Novo Formulário"}</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              ...
              {formType === "answer_key" ? (<div className="space-y-3">{clinicalCases.length === 0 ? <p className="text-sm text-muted-foreground">Cadastre casos clínicos na aba "Casos Clínicos" para configurar espelhos por caso.</p> : (<><div className="flex gap-2 flex-wrap">{clinicalCases.map(c => (<Button key={c.id} variant={activeAnswerKeyCaseId === c.id ? "default" : "outline"} size="sm" onClick={() => setActiveAnswerKeyCaseId(c.id)}>{c.title}</Button>))}</div>{activeAnswerKeyCaseId && <FormBuilder fields={answerKeyByCaseId[activeAnswerKeyCaseId] || []} onChange={(fields) => setAnswerKeyByCaseId(prev => ({ ...prev, [activeAnswerKeyCaseId]: fields }))} />}</>)}</div>)
        ...
        <TabsContent value="cases" className="space-y-4">
          {clinicalCases.map((c: any) => (<Card key={c.id}><CardHeader className="pb-2"><div className="flex items-center justify-between"><CardTitle className="text-base">{c.title}</CardTitle><Button variant="ghost" size="sm" onClick={() => deleteCase(c.id)}><Trash2 className="h-3.5 w-3.5" /></Button></div></CardHeader>{c.content && <CardContent><p className="text-sm whitespace-pre-wrap">{c.content}</p></CardContent>}</Card>))}
