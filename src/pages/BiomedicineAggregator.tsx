...
import { ArrowLeft, Heart, Activity, ArrowRightLeft, ClipboardList, BarChart3 } from "lucide-react";
...
export default function BiomedicineAggregator() {
  const navigate = useNavigate();

  const { data: rooms = [] } = useQuery({
    queryKey: ["biomedicine-agg-rooms"],
    queryFn: async () => {
      const { data: { session } } = await supabase.auth.getSession();
      if (!session) return [];
      const { data } = await supabase.from("biomedicine_rooms").select("id, title, status, module_type").eq("user_id", session.user.id).order("created_at", { ascending: false });
      return (data || []) as RoomInfo[];
    },
  });

  const { data: allResponses = [] } = useQuery({
    queryKey: ["biomedicine-agg-responses"],
    queryFn: async () => {
      const { data } = await supabase.from("biomedicine_responses").select("room_id, pair_index, admin_score, ai_score");
      return data || [];
    },
  });

  const { data: allParticipants = [] } = useQuery({
    queryKey: ["biomedicine-agg-participants"],
    queryFn: async () => {
      const { data } = await supabase.from("biomedicine_participants").select("room_id, pair_index, student_name, student_email, participant_role");
  ...
  const moduleIcons: Record<string, any> = { analise_laboratorial: Heart, controle_qualidade: ClipboardList, interpretacao_resultados: Activity, laudo_tecnico: ArrowRightLeft };

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" onClick={() => navigate("/biomedicine")}><ArrowLeft className="h-4 w-4 mr-1" />Voltar</Button>
        <div>
          <h1 className="text-2xl font-bold">Agregador de Notas — Biomedicina</h1>
