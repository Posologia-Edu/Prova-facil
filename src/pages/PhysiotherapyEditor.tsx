...
export default function PhysiotherapyEditor() {
  ...
  const mt = (moduleType || "avaliacao_funcional") as PhysiotherapyModuleType;
  const navigate = useNavigate();
  const queryClient = useQueryClient();

  const { data: room, isLoading: roomLoading } = useQuery({
    queryKey: ["physiotherapy-room", roomId],
    queryFn: async () => {
      const { data, error } = await supabase.from("physiotherapy_rooms").select("*").eq("id", roomId!).single();
  ...
  const { data: participants = [], refetch: refetchParticipants } = useQuery({
    queryKey: ["physiotherapy-participants", roomId],
    queryFn: async () => {
      const { data, error } = await supabase.from("physiotherapy_participants").select("*").eq("room_id", roomId!).order("pair_index", { ascending: true });
      if (error) throw error;
      return data;
    },
    enabled: !!roomId,
  ...
  const { data: forms = [], refetch: refetchForms } = useQuery({
    queryKey: ["physiotherapy-forms", roomId],
    queryFn: async () => {
      const { data, error } = await supabase.from("physiotherapy_forms").select("*").eq("room_id", roomId!).order("created_at", { ascending: true });
      if (error) throw error;
      return data as any[];
    },
    enabled: !!roomId,
  ...
  const { data: clinicalCases = [], refetch: refetchCases } = useQuery({
    queryKey: ["physiotherapy-clinical-cases", roomId],
    queryFn: async () => {
      const { data, error } = await supabase.from("physiotherapy_clinical_cases").select("*").eq("room_id", roomId!).order("position", { ascending: true });
      if (error) throw error;
      return data;
    },
    enabled: !!roomId,
  ...
  const addParticipant = async () => {
    ...
    const { error } = await supabase.from("physiotherapy_participants").insert({
      room_id: roomId!, student_name: newName.trim(), student_email: newEmail.trim(), pair_index: -1, pair_position: "X", participant_role: "student",
    });
  ...
  const addCase = async () => {
    ...
    const pos = clinicalCases.length;
    await supabase.from("physiotherapy_clinical_cases").insert({ room_id: roomId!, title: newCaseTitle.trim(), content: newCaseContent.trim(), position: pos });
    setNewCaseTitle(""); setNewCaseContent(""); refetchCases();
    toast({ title: "Caso clínico adicionado" });
  };

  const deleteCase = async (id: string) => {
    await supabase.from("physiotherapy_clinical_cases").delete().eq("id", id);
    refetchCases();
  };
  ...
  const saveForm = async (silent = false) => {
    ...
    if (editingFormId) {
      await supabase.from("physiotherapy_forms").update(payload).eq("id", editingFormId);
    } else {
      await supabase.from("physiotherapy_forms").insert({ ...payload, room_id: roomId! });
    }
    ...
    if (!silent) {
      setEditingFormId(null); setFormTitle(""); setFormFields([]); setFormType("standard"); setAnswerKeyByCaseId({}); setActiveAnswerKeyCaseId("");
      refetchForms();
  ...
  const editForm = (form: any) => {
    ...
    if (form.form_type === "answer_key") {
      ...
      if (content?.case_answers) {
        ...
        if (clinicalCases.length > 0) { migrated[clinicalCases[0].id] = content; setActiveAnswerKeyCaseId(clinicalCases[0].id); }
        setAnswerKeyByCaseId(migrated); setFormFields([]);
      ...
      lastSavedSnapshotRef.current = JSON.stringify({ formType: form.form_type, title: form.title, content: form.content_json });
    }
  };

  const deleteForm = async (id: string) => {
    await supabase.from("physiotherapy_forms").delete().eq("id", id);
    refetchForms();
  };
  ...
  const activateRoom = async () => {
    ...
    await supabase.from("physiotherapy_rooms").update({ status: "active" }).eq("id", roomId!);
    toast({ title: "Sala ativada!" });
    queryClient.invalidateQueries({ queryKey: ["physiotherapy-room", roomId] });
  };
  ...
  return (
    <div className="space-y-6">
      <ModuleHelpGuide moduleKey={mt === "cinetico_funcional" ? "cinetico_funcional" : mt === "plano_fisioterapeutico" ? "plano_fisioterapeutico" : mt === "evolucao_fisio" ? "evolucao_fisio" : "avaliacao_funcional"} />
      <div className="flex items-center gap-4">
        <Button variant="ghost" onClick={() => navigate("/physiotherapy")}>
          <ArrowLeft className="h-4 w-4 mr-1" />Voltar
        </Button>
        <div>
          <div className="flex items-center gap-2">
            <Badge variant="outline" className="text-xs bg-cyan-600/10 text-cyan-600 border-cyan-600/30">{moduleLabel[mt]}</Badge>
        ...
        {room.status === "active" && <Button className="ml-auto" onClick={() => navigate(`/physiotherapy/${mt}/control/${roomId}`)}><Play className="h-4 w-4 mr-1" />Painel de Controle</Button>}
      </div>
      ...
      <Tabs defaultValue="participants">
        ...
        <TabsContent value="participants" className="space-y-4">
          ...
          {(() => {
            ...
            const formPair = async () => {
              ...
              await supabase.from("physiotherapy_participants").update({ pair_index: nextPairIdx, pair_position: "A" } as any).eq("id", a);
              await supabase.from("physiotherapy_participants").update({ pair_index: nextPairIdx, pair_position: "B" } as any).eq("id", b);
            ...
            const undoPair = async (pairIdx: number) => {
              ...
              for (const m of members) await supabase.from("physiotherapy_participants").update({ pair_index: -1, pair_position: "X" } as any).eq("id", m.id);
                    ...
                    <CardContent className="p-4 pt-0">
                      <div className="grid grid-cols-2 gap-2">
                        {unpaired.map(p => {
                          ...
                          return (
                            <button key={p.id} onClick={() => toggleSelect(p.id)} className={`p-3 rounded-lg border text-left text-sm transition-colors ${isSelected ? "border-primary bg-primary/10 ring-2 ring-primary" : "border-border hover:border-primary/50"}`}>
        ...
        <TabsContent value="forms" className="space-y-4">
          ...
          <Card>
            <CardHeader><CardTitle className="text-base">{editingFormId ? "Editando Formulário" : "Novo Formulário"}</CardTitle></CardHeader>
            <CardContent className="space-y-4">
              <div className="flex gap-2">
                <div className="flex-1"><Label>Título</Label><Input value={formTitle} onChange={e => setFormTitle(e.target.value)} placeholder="Ex: Ficha de Acolhimento" /></div>
                <div><Label>Tipo</Label>
                  <select className="w-full h-10 border rounded px-3 bg-background" value={formType} onChange={e => setFormType(e.target.value)}>
                    <option value="standard">Formulário</option>
                    <option value="answer_key">Espelho de Respostas</option>
                  </select>
                </div>
              </div>
              ...
              {formType === "answer_key" ? (
                <div className="space-y-3">
                  {clinicalCases.length === 0 ? (
                    <p className="text-sm text-muted-foreground">Cadastre casos clínicos na aba "Casos Clínicos" para configurar espelhos por caso.</p>
                  ) : (
                    <>
                      <div className="flex gap-2 flex-wrap">
                        {clinicalCases.map(c => (
                          <Button key={c.id} variant={activeAnswerKeyCaseId === c.id ? "default" : "outline"} size="sm" onClick={() => setActiveAnswerKeyCaseId(c.id)}>
                            {c.title}
                          </Button>
                        ))}
                      </div>
                      {activeAnswerKeyCaseId && (
                        <FormBuilder fields={answerKeyByCaseId[activeAnswerKeyCaseId] || []} onChange={fields => setAnswerKeyByCaseId(prev => ({ ...prev, [activeAnswerKeyCaseId]: fields }))} />
                      )}
                    </>
                  )}
                </div>
              ...
              <Button onClick={() => saveForm(false)} disabled={!formTitle.trim()}>{editingFormId ? "Salvar Alterações" : "Criar Formulário"}</Button>
            </CardContent>
          </Card>
        </TabsContent>
            ...
            <Card key={c.id}>
              <CardHeader className="pb-2">
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => deleteCase(c.id)}><Trash2 className="h-3.5 w-3.5" /></Button>
                </div>
              </CardHeader>
              <CardContent><p className="text-sm whitespace-pre-wrap text-muted-foreground">{c.content || "Sem conteúdo"}</p></CardContent>
            </Card>
          ))}
          <Card>
            <CardHeader><CardTitle className="text-base">Novo Caso Clínico</CardTitle></CardHeader>
            <CardContent className="space-y-3">
              <div><Label>Título</Label><Input value={newCaseTitle} onChange={e => setNewCaseTitle(e.target.value)} placeholder="Ex: Caso 1 — Paciente com HAS" /></div>
              <div><Label>Conteúdo</Label><Textarea value={newCaseContent} onChange={e => setNewCaseContent(e.target.value)} rows={4} placeholder="Descreva o caso clínico..." /></div>
              <Button onClick={addCase} disabled={!newCaseTitle.trim()}><Plus className="h-4 w-4 mr-1" />Adicionar Caso</Button>
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
