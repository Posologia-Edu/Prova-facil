...
export default function NutritionControl() {
  ...
  const { data: room } = useQuery({ queryKey: ["nutrition-room", roomId], queryFn: async () => { const { data, error } = await supabase.from("nutrition_rooms").select("*").eq("id", roomId!).single(); if (error) throw error; return data; }, enabled: !!roomId });
  const { data: participants = [] } = useQuery({ queryKey: ["nutrition-participants", roomId], queryFn: async () => { const { data, error } = await supabase.from("nutrition_participants").select("*").eq("room_id", roomId!).order("pair_index", { ascending: true }); if (error) throw error; return data; }, enabled: !!roomId, refetchInterval: 5000 });
  const { data: responses = [] } = useQuery({ queryKey: ["nutrition-responses", roomId], queryFn: async () => { const { data, error } = await supabase.from("nutrition_responses").select("*").eq("room_id", roomId!); if (error) throw error; return data as any[]; }, enabled: !!roomId, refetchInterval: 5000 });
  const { data: forms = [] } = useQuery({ queryKey: ["nutrition-forms", roomId], queryFn: async () => { const { data, error } = await supabase.from("nutrition_forms").select("*").eq("room_id", roomId!); if (error) throw error; return data as any[]; }, enabled: !!roomId });
  const { data: clinicalCases = [] } = useQuery({ queryKey: ["nutrition-clinical-cases", roomId], queryFn: async () => { const { data, error } = await supabase.from("nutrition_clinical_cases").select("*").eq("room_id", roomId!).order("position", { ascending: true }); if (error) throw error; return data; }, enabled: !!roomId });
  ...
  const handleSelectPair = (pairIdx: number) => {
    ...
    const resp = responses.find(r => r.pair_index === pairIdx && r.form_id === standardForm?.id);
  ...
  const saveAdminEvaluation = async () => {
    if (selectedResp) await supabase.from("nutrition_responses").update({ admin_score: adminScore ? Number(adminScore) : null, admin_feedback: adminFeedback || null }).eq("id", selectedResp.id);
    toast({ title: "Avaliação salva!" });
    queryClient.invalidateQueries({ queryKey: ["nutrition-responses", roomId] });
  };
  ...
  const gradeWithAI = async () => {
    ...
    setGradingAI(true);
    try {
      const { data, error } = await supabase.functions.invoke("grade-nutrition", {
        body: { room_id: roomId, module_type: mt, pair_index: selectedPairIndex, response: selectedResp ? { id: selectedResp.id, answers_json: selectedResp.answers_json, clinical_case_id: selectedResp.clinical_case_id } : null, answer_key: answerKeyForm?.content_json || null, form_fields: standardForm?.content_json || [] },
      });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" onClick={() => navigate(`/nutrition/${mt}/editor/${roomId}`)}><ArrowLeft className="h-4 w-4 mr-1" />Voltar</Button>
        <div>
          <div className="flex items-center gap-2"><Badge variant="outline" className="text-xs bg-emerald-500/10 text-emerald-500 border-emerald-500/30">{nutritionModuleLabel[mt]}</Badge><h1 className="text-xl font-bold">{room?.title} — Controle</h1></div>
      ...
      <Tabs defaultValue="participants">
        ...
        <TabsContent value="participants" className="space-y-4">
          ...
          {room?.status === "active" && responses.length > 0 && (
            <div className="flex items-center justify-between p-4 border rounded-lg bg-muted/50">
              <div><p className="font-medium text-sm">Encerrar atividade</p><p className="text-xs text-muted-foreground">Conclua a sala.</p></div>
              <Button variant="default" onClick={async () => { await supabase.from("nutrition_rooms").update({ status: "completed" }).eq("id", roomId!); queryClient.invalidateQueries({ queryKey: ["nutrition-room", roomId] }); toast({ title: "Sala concluída!" }); }}><Lock className="h-4 w-4 mr-1" />Concluir Sala</Button>
            </div>
          )}
        ...
        <TabsContent value="responses" className="space-y-4">
          {!pairIndicesWithResponses.length ? <p className="text-sm text-muted-foreground">Nenhuma resposta recebida ainda.</p> : pairIndicesWithResponses.map(pairIdx => {
            ...
            const caseKeyFields = getAnswerKeyFields(caseData?.id);
            return (
              <Card key={pairIdx}><CardHeader><CardTitle className="text-base">{pairNames(pairIdx)}</CardTitle>{caseData && <p className="text-sm text-muted-foreground">Caso: {caseData.title}</p>}</CardHeader>
        ...
        <TabsContent value="grading" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="space-y-2"><h3 className="font-medium text-sm">Selecionar Dupla</h3>
              {pairIndicesWithResponses.map(pairIdx => {
                const resp = responses.find(r => r.pair_index === pairIdx && r.form_id === standardForm?.id);
                return (<Button key={pairIdx} variant={selectedPairIndex === pairIdx ? "default" : "outline"} className="w-full justify-start" size="sm" onClick={() => handleSelectPair(pairIdx)}>{pairNames(pairIdx)}{resp?.admin_score != null && <Badge className="ml-auto" variant="secondary">{resp.admin_score}</Badge>}</Button>);
              })}
            </div>
            <div className="md:col-span-3 space-y-4">
              {selectedPairIndex !== null ? (
                <Card><CardContent className="p-4 space-y-4">
                  <div><Label>Nota do Professor (0-10)</Label><Input type="number" value={adminScore} onChange={e => setAdminScore(e.target.value)} min={0} max={10} step={0.5} /></div>
                  <div><Label>Feedback</Label><Textarea value={adminFeedback} onChange={e => setAdminFeedback(e.target.value)} rows={3} /></div>
                  <div className="flex gap-2"><Button onClick={saveAdminEvaluation}>Salvar Avaliação</Button><Button variant="outline" onClick={gradeWithAI} disabled={gradingAI}>{gradingAI ? <><Loader2 className="h-4 w-4 mr-1 animate-spin" />Corrigindo...</> : <><Bot className="h-4 w-4 mr-1" />Corrigir com IA</>}</Button></div>
                </CardContent></Card>
              ) : <p className="text-sm text-muted-foreground">Selecione uma dupla para avaliar.</p>}
            </div>
          </div>
        </TabsContent>
      </Tabs>
