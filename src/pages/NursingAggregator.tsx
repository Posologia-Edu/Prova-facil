...
export default function NursingAggregator() {
  const navigate = useNavigate();

  const { data: rooms = [] } = useQuery({
    queryKey: ["nursing-agg-rooms"],
    queryFn: async () => {
      const { data: { session } } = await supabase.auth.getSession();
      if (!session) return [];
      const { data } = await supabase.from("nursing_rooms").select("id, title, status, module_type").eq("user_id", session.user.id).order("created_at", { ascending: false });
      return (data || []) as RoomInfo[];
    },
  });

  const { data: allResponses = [] } = useQuery({
    queryKey: ["nursing-agg-responses"],
    queryFn: async () => {
      const { data } = await supabase.from("nursing_responses").select("room_id, pair_index, admin_score, ai_score");
      return data || [];
    },
  });

  const { data: allParticipants = [] } = useQuery({
    queryKey: ["nursing-agg-participants"],
    queryFn: async () => {
      const { data } = await supabase.from("nursing_participants").select("room_id, pair_index, student_name, student_email, participant_role");
  ...
  const moduleIcons: Record<string, any> = { acolhimento: Heart, sae: ClipboardList, evolucao: Activity, passagem_plantao: ArrowRightLeft };

  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" onClick={() => navigate("/nursing")}><ArrowLeft className="h-4 w-4 mr-1" />Voltar</Button>
        <div>
          <h1 className="text-2xl font-bold">Agregador de Notas — Enfermagem</h1>
